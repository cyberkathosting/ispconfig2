...
function maleMenu() {
	...
	ausgabeFrame.write("<A HREF=\"" + menuDaten[1].url + "\" TARGET=\"" + zielFrame + "\" onMouseOver=\"window.status='" + menuDaten[1].rootstat + "'; return true\"><IMG SRC=\"" + bildBezeichnung + "\" WIDTH=16 HEIGHT=16 ALIGN=TOP BORDER=0 ALT=\"" + menuDaten[1].rootstat + "\">&nbsp;<B>" + menuDaten[1].text + "</B></A> &nbsp;<a href=\"javascript:parent.allesaufzu(1)\">[ auf</a> &nbsp; <a href=\"javascript:parent.allesaufzu(0)\">zu ]</a><BR>\n");
	maleVerz("root","");
	if (MenuFett == "1") {
...
function aufzuVerz(name,status) {
    ...
    if (menuDaten[eintragsIndex].url != '') {
    //frames['seiteFrame'].location.href=menuDaten[eintragsIndex].url;
    parent.seiteFrame.location.href=menuDaten[eintragsIndex].url;
    }
 }
...
menuDaten.neu(new HauptverzeichnisEintrag('root','Archiv','frame_start.php?<? echo $session?>&tpl=start','','','Archiv'));
<?
/*******************************************
...
if($mygroups = $go_api->db->queryAllRecords("select * from user_groups, groups where user_groups.userid = '$userid' and user_groups.groupid = groups.groupid and user_groups.userstatus = '1' and groups.groupstatus = '1'")) {
...
foreach($groups as $gid) {
    ...
    if($gid == 0) {
        ...
        $sql= "SELECT * FROM ".$go_info["modul"]["table_name"]."_nodes where userid = '$userid' and status = '1' and parent='root'";
        if ($bookmark_order == "1") $sql = $sql . " order by title";
        ...
        if ($bookmark_order == "1") $sql2 = $sql2 . " order by title";
        $conn2 = mysql_query($sql2);

    } else {

        /*----------- Selektierung der bookmark Daten -----------------------------------------------*/
        $sql= "SELECT * FROM ".$go_info["modul"]["table_name"]."_nodes where groupid = '$gid' and status = '1' and parent='root'";
        if ($bookmark_order == "1") $sql = $sql . " order by title";
        $conn = mysql_query($sql);
    ...
    *******************************************/
    ...
    while($DB2 = mysql_fetch_array($conn2)):
        ...
        if($DB2["type"] == "n" and $groupid == $gid){
            echo "menuDaten.neu(new VerzEintrag('$id','$ordner','$btext','vzzu-0.gif','vzzu-0.gif','listview/list.php?id=$id&$session'));\n";
        }
    endwhile;
    
    // Papierkorb einfügen
    if ($gid == 0) {
    echo "menuDaten.neu(new LinkEintrag('root','Papierkorb','edit/papierkorb.php?gid=$gid&amp;$session','seiteFrame','papierkorb.gif','Papierkorb','i','$session'));\n";
    } else {
    ...
    while($DB = mysql_fetch_array($conn)):
        ...
        if ($art == "i" and $groupid == $gid):
           echo "menuDaten.neu(new LinkEintrag('$ordner','$btext','edit/edit.php?tree_id=$id&amp;$session','seiteFrame','$icon','','$art','$id&amp;$session'));\n";
