...
menuDaten.neu(new HauptverzeichnisEintrag('root','<? echo $go_api->lng("ISP Faktura")?>','frame_start.php?<? echo $session?>&tpl=start','','','Archiv'));
<?

/*******************************************
* Gruppen Bookmark Einträge erzeugen
*******************************************/
...
if($mygroups = $go_api->db->queryAllRecords("select * from user_groups, groups where user_groups.userid = '$userid' and user_groups.groupid = groups.groupid and user_groups.userstatus = '1' and groups.groupstatus = '1' order by groups.name")) {
...
foreach($groups as $gid) {
    ...
    if($gid == 0) {
        ...
        /*----------- Selektierung der bookmark Daten -----------------------------------------------*/
        $sql= "SELECT * FROM ".$go_info["modul"]["table_name"]."_nodes where groupid = '$gid' and status = '1'";
        if ($bookmark_order == "j") $sql = $sql . " order by title";
        $conn = mysql_query($sql);

        /*----------- Selektierung der bookmark Ordner -----------------------------------------------*/
        $sql2= "SELECT * FROM ".$go_info["modul"]["table_name"]."_nodes where type = 'n' and groupid = '$gid' and status = '1'";
        if ($bookmark_order == "j") $sql2 = $sql2 . " order by title";
        $conn2 = mysql_query($sql2);
    }

    /*******************************************
    * Bookmark Einträge erzeugen
    *******************************************/
    ...
    while($DB2 = mysql_fetch_array($conn2)) {
        ...
        $btext = addslashes($go_api->lng($go_api->lng($DB2["title"])));
        $groupid = $DB2["groupid"];
    ...
    echo "menuDaten.neu(new LinkEintrag('group$gid','".$go_api->lng("Papierkorb")."','../multidoc/edit/papierkorb.php?gid=$gid&amp;$session','seiteFrame','papierkorb.gif','Papierkorb','i','$session'));\n";
    }
    ...
    while($DB = mysql_fetch_array($conn)) {
        ...
        if ($art == "i" and $groupid == $gid):
           echo "menuDaten.neu(new LinkEintrag('$ordner','$btext','../multidoc/edit/edit.php?tree_id=$id&amp;$session','seiteFrame','$icon','','$art','$id&amp;$session'));\n";
...
echo "menuDaten.neu(new VerzEintrag('rechnungen','root','".$go_api->lng("Rechnungen")."','vzzu-0.gif','vzauf-0.gif',''));\n";

echo "menuDaten.neu(new LinkEintrag('rechnungen','".$go_api->lng("Kunden")."','view/client.php?$session','seiteFrame','doc.gif','','$art','$id&amp;$session'));\n";
echo "menuDaten.neu(new LinkEintrag('rechnungen','".$go_api->lng("Anbieter")."','view/reseller.php?$session','seiteFrame','doc.gif','','$art','$id&amp;$session'));\n";


//echo "menuDaten.neu(new VerzEintrag('versand','root','".$go_api->lng("PDF Versand")."','vzzu-0.gif','vzauf-0.gif',''));\n";


//echo "menuDaten.neu(new VerzEintrag('tools','root','".$go_api->lng("Import / Export")."','vzzu-0.gif','vzauf-0.gif',''));\n";
...
?>
 ...
 standardZielFrame = 'seiteFrame';          // Name des Frames, in den standardmäßig alle Links geladen werden
  ...
  standardLinkBild = 'doc.gif';          // Name des standardmäßig benutzten Icons für Links
