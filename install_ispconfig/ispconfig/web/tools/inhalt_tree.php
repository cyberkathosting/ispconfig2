...
menuDaten.neu(new HauptverzeichnisEintrag('root','<? echo $go_api->lng("Tools & Optionen")?>','frame_start.php?<? echo $session?>&tpl=start','','','Tools'));

<!-- Module Management //-->

//menuDaten.neu(new VerzEintrag('modules','root','Modul Management','','',''));
//menuDaten.neu(new LinkEintrag('modules','Hinzufügen','news/topnews.php?<?echo $session?>','seiteFrame','','Modul Hinzufügen','n','<?echo "1&amp;$session"?>'));
//menuDaten.neu(new LinkEintrag('modules','Alle anzeigen','news/topnews.php?<?echo $session?>','seiteFrame','','Module anzeigen','n','<?echo "1&amp;$session"?>'));

<!-- User Management //-->

//menuDaten.neu(new VerzEintrag('user','root','User Management','','',''));
//menuDaten.neu(new LinkEintrag('user','Hinzufügen','../multidoc/edit/edit.php?doctype_id=7&<?echo $session?>','seiteFrame','','User Hinzufügen','n','<?echo "1&amp;$session"?>'));
//menuDaten.neu(new LinkEintrag('user','Alle anzeigen','user/user_list.php?<?echo $session?>','seiteFrame','','User anzeigen','n','<?echo "1&amp;$session"?>'));
//menuDaten.neu(new LinkEintrag('user','Suchen','news/topnews.php?<?echo $session?>','seiteFrame','','User suchen','n','<?echo "1&amp;$session"?>'));

<!-- Gruppen Management //-->

//menuDaten.neu(new VerzEintrag('groups','root','Gruppen Management','','',''));
//menuDaten.neu(new LinkEintrag('groups','Hinzufügen','../messenger/groups/group_edit.php?<?echo $session?>','seiteFrame','','Gruppe Hinzufügen','n','<?echo "1&amp;$session"?>'));
//menuDaten.neu(new LinkEintrag('groups','Alle anzeigen','news/topnews.php?<?echo $session?>','seiteFrame','','Gruppe anzeigen','n','<?echo "1&amp;$session"?>'));
//menuDaten.neu(new LinkEintrag('groups','Suchen','news/topnews.php?<?echo $session?>','seiteFrame','','Gruppe suchen','n','<?echo "1&amp;$session"?>'));

menuDaten.neu(new VerzEintrag('all','root','<? echo $go_api->lng("Allgemein")?>','','',''));
//menuDaten.neu(new LinkEintrag('all','easyLogin','tools/easylogin.php?<?echo $session?>','seiteFrame','','easyLogin','n','<?echo "1&amp;$session"?>'));
menuDaten.neu(new LinkEintrag('all','<? echo $go_api->lng("Passwort ändern")?>','options/passwort.php?<? echo $session?>','seiteFrame','lock.gif','<? echo $go_api->lng("Passwort ändern")?>','n','<? echo "1&amp;$session"?>'));
//menuDaten.neu(new LinkEintrag('all','<? echo $go_api->lng("Adresse ändern")?>','options/nutzerdaten.php?<? echo $session?>','seiteFrame','','Nutzerdaten','n','<? echo "1&amp;$session"?>'));
menuDaten.neu(new LinkEintrag('all','<? echo $go_api->lng("Sprache ändern")?>','options/language.php?<? echo $session?>','seiteFrame','cabinet.gif','<? echo $go_api->lng("Sprache ändern")?>','n','<? echo "1&amp;$session"?>'));


<?
// Modul Tools einbinden
while (list($key, $val) = each($go_info["session"]["modules"])){
    $inhalt = $go_info["server"]["server_root"]. $go_info["server"]["dir_trenner"] . "web" . $go_info["server"]["dir_trenner"] .$val["path"] . $go_info["server"]["dir_trenner"].'lib'.$go_info["server"]["dir_trenner"].'tools.inc.php';
...
?>
//menuDaten.neu(new LinkEintrag('root','easyLogin','easylogin.php?<?echo $session?>','seiteFrame','','easyLogin','n','<?echo "1&amp;$session"?>'));
